...
class RBM(BernoulliRBM):
    ...
    def _fit(self, v_pos, t_pos, rng):
        ...
        if (self.target_in_model):
            ...
            self.target_components_ += lr * update_target
            self.intercept_target_ += lr * (t_pos.sum(axis=0) - t_neg.sum(axis=0))

        if (self.latent_dist == 'multinomial'):
            p = softmax(h_neg)
            self.h_samples_ = [rng.multinomial(self.sample_size, pval) for pval in p]
